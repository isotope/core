 ...
 * @package    Controller
 */
class DC_TablePageId extends DataContainer implements listable, editable
{
	...
	public function __construct($strTable)
	{
		...
		if ($this->treeView)
		{
			...
			{
				$objIds = $this->Database->prepare("SELECT id FROM " . $table . " WHERE page_id=?" . ($this->Database->fieldExists('sorting', $strTable) ? ' ORDER BY sorting' : ''))
										 ->execute(0);
...
';
	...
	public function show()
	{
		...
		$allowedFields = array('id', 'page_id', 'sorting', 'tstamp');
...
<table cellpadding="0" cellspacing="0" class="tl_show" summary="Table lists all details of an entry">'.$return.'
	...
	public function create($set=array())
	{
		...
		$this->getNewPosition('new', (strlen($this->Input->get('page_id')) ? $this->Input->get('page_id') : null), ($this->Input->get('mode') == '2' ? true : false));
	...
	public function copy($blnDoNotRedirect=false)
	{
		...
		// Get the new position
		$this->getNewPosition('copy', (strlen($this->Input->get('page_id')) ? $this->Input->get('page_id') : null), ($this->Input->get('mode') == '2' ? true : false));
	...
	protected function copyChilds($table, $insertID, $id, $parentId)
	{
		...
		if (!$GLOBALS['TL_DCA'][$table]['config']['ptable'] && strlen($this->Input->get('childs')) && $this->Database->fieldExists('page_id', $table) && $this->Database->fieldExists('sorting', $table))
		{
			$ctable[] = $table;
		}
		...
		foreach ($ctable as $v)
		{
			...
			{
				$objCTable = $this->Database->prepare("SELECT * FROM " . $v . " WHERE page_id=?" . ($this->Database->fieldExists('sorting', $v) ? " ORDER BY sorting" : ""))
											->execute($id);
				...
				foreach ($objCTable->fetchAllAssoc() as $row)
				{
					...
					$copy[$v][$row['id']]['page_id'] = $insertID;
	...
	public function delete($blnDoNotRedirect=false)
	{
		...
		// If there is a page_id field but no parent table
		if ($this->Database->fieldExists('page_id', $this->strTable) && !strlen($this->ptable))
		{
		...
		$objUndoStmt = $this->Database->prepare("INSERT INTO tl_undo (page_id, tstamp, fromTable, query, affectedRows, data) VALUES (?, ?, ?, ?, ?, ?)")
	...
	public function deleteChilds($table, $id, &$delete)
	{
		...
		foreach ($ctable as $v)
		{
			...
			$objDelete = $this->Database->prepare("SELECT id FROM " . $v . " WHERE page_id=?")
										->execute($id);
	...
	public function edit($intID=false, $ajaxId=false)
	{
		...
		{
			$objData = $this->Database->prepare("SELECT * FROM tl_version WHERE fromTable=? AND page_id=? AND version=?")
									  ->limit(1)
			...
			if ($objData->numRows)
			{
				...
				if (is_array($data))
				{
					...
					$this->Database->prepare("UPDATE tl_version SET active='' WHERE page_id=?")
								   ->execute($this->intId);

					$this->Database->prepare("UPDATE tl_version SET active=1 WHERE page_id=? AND version=?")
		...
		{
			$objVersion = $this->Database->prepare("SELECT id FROM tl_version WHERE fromTable=? AND page_id=?")
										 ->limit(1)
		...
		if ($GLOBALS['TL_DCA'][$this->strTable]['config']['enableVersioning'])
		{
			$objVersion = $this->Database->prepare("SELECT tstamp, version, username, active FROM tl_version WHERE fromTable=? AND page_id=? ORDER BY version DESC")
									     ->execute($this->strTable, $this->intId);
...
<p class="tl_error">'.$GLOBALS['TL_LANG']['ERR']['general'].'</p>' : '').$return;
		...
		if ($this->Input->post('FORM_SUBMIT') == $this->strTable && !$this->noReload)
		{
			...
			elseif (isset($_POST['saveNcreate']))
			{
				...
				if ($this->treeView)
				{
					$strUrl .= '&amp;act=create&amp;mode=1&amp;page_id=' . $this->intId;
				}

				// Parent view
				elseif ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 4)
				{
					$strUrl .= $this->Database->fieldExists('sorting', $this->strTable) ? '&amp;act=create&amp;mode=1&amp;page_id=' . $this->intId . '&amp;id=' . CURRENT_ID : '&amp;act=create&amp;mode=2&amp;page_id=' . CURRENT_ID;
				}

				// List view
				else
				{
					$strUrl .= strlen($GLOBALS['TL_DCA'][$this->strTable]['config']['ptable']) ? '&amp;act=create&amp;mode=2&amp;page_id=' . CURRENT_ID : '&amp;act=create';
				}
		...
		if (is_array($fields) && count($fields) && $this->Input->get('fields'))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($this->User->isAdmin)
				{
					...
					if ($this->Database->fieldExists('page_id', $this->strTable))
					{
						array_unshift($this->strPalette, 'page_id');
					}

					$GLOBALS['TL_DCA'][$this->strTable]['fields']['page_id'] = array('label'=>&$GLOBALS['TL_LANG']['MSC']['page_id'], 'inputType'=>'text', 'eval'=>array('rgxp'=>'digit'));
...
//--><!]]>
		...
		else
		{
			...
			if ($this->User->isAdmin)
			{
				...
				if ($this->Database->fieldExists('page_id', $this->strTable) && !in_array('page_id', $fields))
				{
					array_unshift($fields, 'page_id');
				}
			}
			...
			{
				if ($field == 'page_id' || $field == 'sorting' || (!$GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['exclude'] && !$GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['doNotShow'] && (strlen($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['inputType']) || is_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['input_field_callback']))))
				{
					$options .= '
...
<a href="'.$this->getReferer(true).'" class="header_back" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['backBT']).'" accesskey="b" onclick="Backend.getScrollOffset();">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
	...
	protected function treeView()
	{
		...
		if ($this->Input->get('ptg') == 'all')
		{
			...
			{
				$session[$node] = array();
				$objNodes = $this->Database->execute("SELECT DISTINCT page_id FROM " . $table . " WHERE page_id>0");

				while ($objNodes->next())
				{
					$session[$node][$objNodes->page_id] = 1;
				}
			}
		...
		// Return if a mandatory field (id, page_id, sorting) is missing
		if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && (!$this->Database->fieldExists('id', $table) || !$this->Database->fieldExists('page_id', $table) || !$this->Database->fieldExists('sorting', $table)))
		{
			return '
  ...
  <li class="tl_folder_top" onmouseover="Theme.hoverDiv(this, 1);" onmouseout="Theme.hoverDiv(this, 0);"><div class="tl_left">'.$label.'</div> <div class="tl_right">';
		...
		if ($this->Input->get('act') != 'select' && $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5 && $blnClipboard && !count($GLOBALS['TL_DCA'][$table]['list']['sorting']['root']) && $GLOBALS['TL_DCA'][$table]['list']['sorting']['root'] !== false)
		{
			...
			else
			{
				...
				$_buttons = '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=2&amp;page_id=0'.(!is_array($arrClipboard['id']) ? '&amp;id='.$arrClipboard['id'] : '')).'" title="'.specialchars($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][0]).'" onclick="Backend.getScrollOffset();">'.$imagePasteInto.'</a> ';
			}
		}
...
<div class="tl_submit_container">
  ...
  <input type="submit" name="edit" id="edit" class="tl_submit" alt="edit selected records" accesskey="s" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['editSelected']).'" />' : '') . '
	...
	public function ajaxTreeView($id, $level)
	{
		...
		// Get records
		$objRows = $this->Database->prepare("SELECT id FROM " . $table . " WHERE page_id=?" . ($hasSorting ? " ORDER BY sorting" : ""))
							 	  ->execute($id);
	...
	protected function generateTree($table, $id, $arrPrevNext, $blnHasSorting, $intMargin=0, $arrClipboard=false, $blnCircularReference=false, $protectedPage=false)
	{
		...
		{
			$objChilds = $this->Database->prepare("SELECT id FROM " . $table . " WHERE page_id=?" . ($blnHasSorting ? " ORDER BY sorting" : ''))
										->execute($id);
		...
		if ($arrClipboard !== false && $this->Input->get('act') != 'select')
		{
			...
			else
			{
				...
				{
					$_buttons .= ($arrClipboard['mode'] == 'cut' && ($blnCircularReference || $arrClipboard['id'] == $id) || $arrClipboard['mode'] == 'cutAll' && ($blnCircularReference || in_array($id, $arrClipboard['id'])) || (count($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['root']) && in_array($id, $this->root))) ? $this->generateImage('pasteafter_.gif', '', 'class="blink"').' ' : '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=1&amp;page_id='.$id.(!is_array($arrClipboard['id']) ? '&amp;id='.$arrClipboard['id'] : '')).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteafter'][1], $id)).'" onclick="Backend.getScrollOffset();">'.$imagePasteAfter.'</a> ';
					$_buttons .= ($arrClipboard['mode'] == 'paste' && ($blnCircularReference || $arrClipboard['id'] == $id) || $arrClipboard['mode'] == 'cutAll' && ($blnCircularReference || in_array($id, $arrClipboard['id']))) ? $this->generateImage('pasteinto_.gif', '', 'class="blink"').' ' : '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=2&amp;page_id='.$id.(!is_array($arrClipboard['id']) ? '&amp;id='.$arrClipboard['id'] : '')).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][1], $id)).'" onclick="Backend.getScrollOffset();">'.$imagePasteInto.'</a> ';
				}

				// Extended tree
				else
				{
					$_buttons .= ($this->strTable == $table) ? (($arrClipboard['mode'] == 'cut' && ($blnCircularReference || $arrClipboard['id'] == $id) || $arrClipboard['mode'] == 'cutAll' && ($blnCircularReference || in_array($id, $arrClipboard['id']))) ? $this->generateImage('pasteafter_.gif', '', 'class="blink"') : '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=1&amp;page_id='.$id.(!is_array($arrClipboard['id']) ? '&amp;id='.$arrClipboard['id'] : '')).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteafter'][1], $id)).'" onclick="Backend.getScrollOffset();">'.$imagePasteAfter.'</a> ') : '';
					$_buttons .= ($this->strTable != $table) ? '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=2&amp;page_id='.$id.(!is_array($arrClipboard['id']) ? '&amp;id='.$arrClipboard['id'] : '')).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][1], $id)).'" onclick="Backend.getScrollOffset();">'.$imagePasteInto.'</a> ' : '';
				}
			}
		}
		...
		{
			$objChilds = $this->Database->prepare("SELECT id FROM " . $this->strTable . " WHERE page_id=?" . ($blnHasSorting ? " ORDER BY sorting" : ''))
							 			->execute($id);
...
<input type="text" name="tl_value" class="tl_text' . ($active ? ' active' : '') . '" value="'.specialchars($session['search'][$this->strTable]['value']).'" />
	...
	protected function sortMenu()
	{
		...
		// Add page_id to order fields
		if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 3 && $this->Database->fieldExists('page_id', $this->strTable))
		{
			array_unshift($orderBy, 'page_id');
		}
