...
class DC_MediaUpload extends DataContainer implements listable, editable
{
	...
	public function __construct($strTable)
	{
		...
		{
			$this->log('Could not load data container configuration for "' . $strTable . '"', 'DC_MediaUpload __construct()', TL_ERROR);
		...
		{
			$this->log('Attempt to create new folder although the method has been overwritten in the data container', 'DC_MediaUpload __construct()', TL_ERROR);
		...
		if ($GLOBALS['TL_DCA'][$this->strTable]['config']['validFileTypes'])
		{
			...
			if ($this->Input->get('id'))
			{
				...
				{
					$this->log('File "'.$this->Input->get('id').'" is not an allowed file type', 'DC_MediaUpload __construct()', TL_ERROR);
					$this->redirect('typolight/main.php?act=error');
				}
			}
		}
		...
		if ($this->Input->get('id') && !preg_match('/^'.preg_quote($GLOBALS['TL_CONFIG']['isotope_upload_path'], '/').'/i', $this->Input->get('id', DECODE_ENTITIES)) && !strlen($this->Input->get('special', DECODE_ENTITIES)))
		{
			$this->log('File or folder "'.$this->Input->get('id').'" is not in the files directory', 'DC_MediaUpload __construct()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}

		
		if ($this->Input->get('pid') && !preg_match('/^'.preg_quote($GLOBALS['TL_CONFIG']['isotope_upload_path'], '/').'/i', $this->Input->get('pid'))  && !strlen($this->Input->get('special', DECODE_ENTITIES)))
		{
			$this->log('File or folder "'.$this->Input->get('pid').'" is not in the files directory', 'DC_MediaUpload __construct()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		if (is_array($GLOBALS['TL_DCA'][$strTable]['list']['sorting']['root']))
		{
			...
			{
				$this->log('Attempt to edit, copy, move or delete root folder "'.$this->Input->get('id').'"', 'DC_MediaUpload __construct()', TL_ERROR);
				$this->redirect('typolight/main.php?act=error');
			}
		}
	}
	...
	public function showAll()
	{
		...
		if ($this->Input->get('tg') == 'all')
		{
			...
			// Expand tree
			if (!is_array($session['isotope_filetree']) || count($session['isotope_filetree']) < 1 || current($session['isotope_filetree']) != 1)
			{
				$session['isotope_filetree'] = $this->getMD5Folders('isotope');
			}

			// Collapse tree
			else
			{
				$session['isotope_filetree'] = array();
			}
		...
		{
			$return .= $this->generateTree(TL_ROOT . '/' .$GLOBALS['TL_CONFIG']['isotope_upload_path'], 0);
		}
		else
		{
			//echo 'yes';
...
<ul class="tl_listing">
  <li class="tl_folder_top" onmouseover="Theme.hoverDiv(this, 1);" onmouseout="Theme.hoverDiv(this, 0);"><div class="tl_left">'.$this->generateImage('filemounts.gif').' '.$GLOBALS['TL_LANG']['MSC']['filetree'].'</div> <div class="tl_right">'.(($this->Input->get('act') == 'paste' && !count($this->arrFilemounts) && !is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['root']) && $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['root'] !== false) ? '<a href="'.$this->addToUrl('&amp;act='.$this->Input->get('mode').'&amp;mode=2&amp;pid='.$GLOBALS['TL_CONFIG']['isotope_upload_path'].'&amp;id='.$this->Input->get('id')).'" title="'.specialchars($GLOBALS['TL_LANG'][$this->strTable]['pasteinto'][1]).'" onclick="Backend.getScrollOffset();">'.$imagePasteInto.'</a>' : '&nbsp;').'</div><div style="clear:both;"></div></li>'.$return.'
</ul>
...
<div class="tl_submit_container">
  ...
  <input type="submit" name="edit" id="edit" class="tl_submit" alt="edit selected records" accesskey="s" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['editSelected']).'" />' : '') . '
	...
	public function create()
	{
		...
		{
			$this->log('Folder "'.$this->Input->get('pid').'" was not mounted or is not a directory', 'DC_MediaUpload create()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
	...
	public function cut()
	{
		...
		{
			$this->log('File or folder "'.$this->Input->get('id').'" was not mounted or could not be found', 'DC_MediaUpload cut()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}

		if (!file_exists(TL_ROOT . '/' . $this->Input->get('pid', DECODE_ENTITIES)) || !$this->isMounted($this->Input->get('pid', DECODE_ENTITIES)))
		{
			$this->log('Folder "'.$this->Input->get('pid').'" was not mounted or is not a directory', 'DC_MediaUpload cut()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		{
			$this->log('Attempt to move folder "'.$this->Input->get('id').'" to "'.$this->Input->get('pid').'" (circular reference)!', 'DC_MediaUpload cut()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		$this->log('File or folder "' . $source . '" has been moved to "' . $destination . '"', 'DC_MediaUpload cut()', TL_FILES);
	...
	public function copy($source='', $destination='')
	{
		...
		if (!file_exists(TL_ROOT . '/' . $this->Input->get('id', DECODE_ENTITIES)) || !$this->isMounted($this->Input->get('id', DECODE_ENTITIES)))
		{
			$this->log('File or folder "'.$this->Input->get('id').'" was not mounted or could not be found', 'DC_MediaUpload copy()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}

		if (!file_exists(TL_ROOT . '/' . $this->Input->get('pid', DECODE_ENTITIES)) || !$this->isMounted($this->Input->get('pid', DECODE_ENTITIES)))
		{
			$this->log('Folder "'.$this->Input->get('pid').'" was not mounted or is not a directory', 'DC_MediaUpload copy()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		if (preg_match('/^' . preg_quote($this->Input->get('id', DECODE_ENTITIES), '/') . '/i', $this->Input->get('pid', DECODE_ENTITIES)))
		{
			$this->log('Attempt to copy folder "'.$this->Input->get('id').'" to "'.$this->Input->get('pid').'" (circular reference)!', 'DC_MediaUpload copy()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		if (!$noReload)
		{
			...
			{
				$this->log('File or folder "'.$this->Input->get('id').'" has been duplicated', 'DC_MediaUpload copy()', TL_FILES);
			}
	 ...
	 * @param string
	 */
	public function delete($source='', $special='')
	{
		$noReload = strlen($source);
		...
		// Get special isotope actions
		$special = strlen($special) ? $special : $this->Input->get('special', DECODE_ENTITIES);
		...
		if(strlen($special))
		{
			...
			//GET AN ARRAY OF ALL OCCURRENCES OF WHERE A FILENAME COULD BE IN ISOTOPE (this should be a global function)
			$arrRelImageLocations = array();
			$arrRelImageLocations[0] = $GLOBALS['TL_CONFIG']['isotope_base_path'] . '/' . $special . '/' . $GLOBALS['TL_LANG']['MSC']['gallery_thumbnail_images_folder'] . '/' . $source;
			$arrRelImageLocations[1] = $GLOBALS['TL_CONFIG']['isotope_base_path'] . '/' . $special . '/' . $GLOBALS['TL_LANG']['MSC']['thumbnail_images_folder'] . '/' . $source;
			$arrRelImageLocations[2] = $GLOBALS['TL_CONFIG']['isotope_base_path'] . '/' . $special . '/' . $GLOBALS['TL_LANG']['MSC']['medium_images_folder'] . '/' . $source;
			$arrRelImageLocations[3] = $GLOBALS['TL_CONFIG']['isotope_base_path'] . '/' . $special . '/' . $GLOBALS['TL_LANG']['MSC']['large_images_folder'] . '/' . $source;
			$arrRelImageLocations[4] = $GLOBALS['TL_LANG']['MSC']['assetsImportBasePath'] . '/' . $special . '/' . $source;
			$arrRelImageLocations[5] = $GLOBALS['TL_LANG']['MSC']['assetsImportBasePath'] . '/' . $special . '/tmp/' . $source;
			
			//NOW THAT WE HAVE OUR ARRAY, WE CAN LOOP THROUGH AND DELETE OCCURANCES
			for($i=0;$i<count($arrRelImageLocations);$i++)
			{
				if (!file_exists($GLOBALS['TL_CONFIG']['isotope_root'] . '/' . $arrRelImageLocations[$i]))
				{
					$this->log('File "' . $GLOBALS['TL_CONFIG']['isotope_upload_path'] . '/' . $arrRelImageLocations[$i] . '" could not be found', 'DC_MediaUpload delete()', TL_ERROR);
				} 
				
				else
				{
				  $this->Files->delete($GLOBALS['TL_CONFIG']['isotope_upload_path'] . '/' . $arrRelImageLocations[$i]);
				  $this->log('File "' . $GLOBALS['TL_CONFIG']['isotope_upload_path'] . '/' . $arrRelImageLocations[$i] . '" has been deleted', 'DC_MediaUpload delete()', TL_FILES);
				}
			}
		...
		
		{
				...
				{
					$this->log('File or folder "' . $source . '" was not mounted or could not be found', 'DC_MediaUpload delete()', TL_ERROR);
					$this->redirect('typolight/main.php?act=error');
				}
				...
				if (!$noReload)
				{
					$this->log('File or folder "' . str_replace(TL_ROOT.'/', '', $source) . '" has been deleted', 'DC_MediaUpload delete()', TL_FILES);
					$this->redirect($this->getReferer());
				}
	...
	public function move()
	{
		...
		if (!file_exists(TL_ROOT . '/' . $this->Input->get('pid', DECODE_ENTITIES)) || !$this->isMounted($this->Input->get('pid', DECODE_ENTITIES)))
		{
			$this->log('Folder "' . $this->Input->get('pid') . '" was not mounted or is not a directory', 'DC_MediaUpload move()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
			...
			foreach ($_FILES as $file)
			{
				...
				if (!is_uploaded_file($file['tmp_name']))
				{
					if (in_array($file['error'], array(1, 2)))
					{
						...
						$this->log('File "'.$file['name'].'" exceeds the maximum file size of '.$maxlength_kb.' kB' , 'DC_MediaUpload move()', TL_ERROR);
					...
					if ($file['error'] == 3)
					{
						...
						$this->log('File "'.$file['name'].'" was only partially uploaded' , 'DC_MediaUpload move()', TL_ERROR);
				...
				if (!in_array(strtolower($pathinfo['extension']), $uploadTypes))
				{
					...
					$this->log('File type "'.$pathinfo['extension'].'" is not allowed to be uploaded ('.$file['name'].')', 'DC_MediaUpload move()', TL_ERROR);
				...
				if ($this->Files->move_uploaded_file($file['tmp_name'], $strNewFile))
				{
					...
					if ($blnResized)
					{
						...
						$this->log('File "'.$file['name'].'" uploaded and resized successfully', 'DC_MediaUpload move()', TL_FILES);
					}
					else
					{
						$_SESSION['TL_CONFIRM'][] = sprintf($GLOBALS['TL_LANG']['MSC']['fileUploaded'], $file['name']);
						$this->log('File "'.$file['name'].'" uploaded successfully', 'DC_MediaUpload move()', TL_FILES);
					}
				}
			}
...
<input type="submit" name="uploadNback" class="tl_submit" alt="upload files and go back" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG'][$this->strTable]['uploadNback']).'" />
	...
	public function edit()
	{
		...
		if (!file_exists(TL_ROOT . '/' . $this->Input->get('id', DECODE_ENTITIES)) || !$this->isMounted($this->Input->get('id', DECODE_ENTITIES)))
		{
			$this->log('File or folder "'.$this->Input->get('id').'" was not mounted or could not be found', 'DC_MediaUpload edit()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
...
//--><!]]>
	...
	public function editAll()
	{
		...
		{
			$this->log('Table ' . $this->strTable . ' is not editable', 'DC_MediaUpload editAll()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
...
<a href="'.$this->getReferer(ENCODE_AMPERSANDS).'" class="header_back" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['backBT']).'" accesskey="b" onclick="Backend.getScrollOffset();">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
	...
	public function source()
	{
		...
		{
			$this->log('Directory "'.$this->intId.'" cannot be edited', 'DC_MediaUpload source()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		{
			$this->log('Not enough permissions to edit file source of file "'.$this->intId.'"', 'DC_MediaUpload source()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		{
			$this->log('File type "'.$objFile->extension.'" ('.$this->intId.') is not allowed to be edited', 'DC_MediaUpload source()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
...
<input type="submit" name="saveNclose" id="saveNclose" class="tl_submit" alt="save all changes and return" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['saveNclose']).'" />
	...
	public function protect()
	{
		if (!is_dir(TL_ROOT . '/' . $this->intId))
		{
			$this->log('Resource "' . $this->intId . '" is not a directory', 'DC_MediaUpload protect()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		...
		if (file_exists(TL_ROOT . '/' . $this->intId . '/.htaccess'))
		{
			...
			$this->log('Protection from folder "' . $this->intId . '" has been removed', 'DC_MediaUpload protect()', TL_FILES);
			$this->redirect($this->getReferer());
		}
		...
		else
		{
			...
			$this->log('Folder "' . $this->intId . '" has been protected', 'DC_MediaUpload protect()', TL_FILES);
			$this->redirect($this->getReferer());
		}
	}
	...
	protected function save($varValue)
	{
		...
		{
			$this->log('Folder "'.$this->strPath.'/'.$varValue.$this->strExtension.'" has been created', 'DC_MediaUpload save()', TL_FILES);
		}
		else
		{
			$this->log('File or folder "'.$this->strPath.'/'.$this->varValue.$this->strExtension.'" has been renamed to "'.$this->strPath.'/'.$varValue.$this->strExtension.'"', 'DC_MediaUpload save()', TL_FILES);
		}
	...
	protected function generateTree($path, $intMargin, $mount=false, $blnProtected=false)
	{
		...
		{
			$session['isotope_filetree'][$this->Input->get('tg')] = (isset($session['isotope_filetree'][$this->Input->get('tg')]) && $session['isotope_filetree'][$this->Input->get('tg')] == 1) ? 0 : 1;
			$this->Session->setData($session);
		...
		for ($f=0; $f<count($folders); $f++)
		{
			...
			$session['isotope_filetree'][$md5] = is_numeric($session['isotope_filetree'][$md5]) ? $session['isotope_filetree'][$md5] : 0;

			// Add a toggle button if there are childs
			if ($countFiles > 0)
			{
				$folderAttribute = '';
				$img = ($session['isotope_filetree'][$md5] == 1) ? 'folMinus.gif' : 'folPlus.gif';
				$alt = ($session['isotope_filetree'][$md5] == 1) ? $GLOBALS['TL_LANG']['MSC']['collapseNode'] : $GLOBALS['TL_LANG']['MSC']['expandNode'];
			...
			$folderImg = ($session['isotope_filetree'][$md5] == 1 && $countFiles > 0) ? ($protected ? 'folderOP.gif' : 'folderO.gif') : ($protected ? 'folderCP.gif' : 'folderC.gif');
			...
			// Call next node
			if ($countFiles > 0 && $session['isotope_filetree'][$md5] == 1)
			{
