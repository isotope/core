...
class tl_product_dimension_prices extends Backend
{
	...
	public function listPrice($row)
	{
		...
		return sprintf('<div class="list_icon" style="margin-top:-18px; margin-bottom:-6px; background-image:url(\'system/themes/%s/images/%s.gif\');">%s x %s: %s%s</div>', $this->getTheme(), $image, round($row['dimension_x'], 3), round($row['dimension_y'], 3), $this->Isotope->formatPriceWithCurrency($row['price']), $strStartStop);
	}
}
