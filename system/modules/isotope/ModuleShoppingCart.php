...
class ModuleShoppingCart extends ModuleIsotopeBase
{
	...
	{		
		...
		switch($strAction)
		{
			case 'add_to_cart':
				...
				$this->addToCart($this->Input->get('id'), $intAttributeSetId, $this->Input->get('quantity_requested'));
				$this->blnRecallProductData = true;
				break;
			case 'update_cart':
				$intAttributeSetId = $this->getAttributeSetId($this->Input->get('aset_id'));

				$this->updateCart($this->Input->get('id'), $intAttributeSetId, $this->Input->get('quantity_requested'));
				$this->blnRecallProductData = true;
				break;
			case 'remove_from_cart':
				...
				$this->updateCart($this->Input->get('id'), $this->Input->get('attribute_set_id'), 0);
				$this->blnRecallProductData = true;
				break;
			default:
		...
		if($this->Input->post('form_action')=='cart_update')
		{
			...
			{
				$this->updateCart($k, $v, $this->Input->post('product_qty_' . $k), true);
	...
	protected function formatProductData($arrProductData)
	{
 		...
 		foreach($arrProductData as $row)
		{
			...
			(
				...
				'remove_link'		=> $this->generateActionLinkString('remove_from_cart', $row['product_id'], array('attribute_set_id'=>$row['attribute_set_id'],'quantity'=>0), $objPage->id),
	 ...
	 * @return boolean
	 */
	protected function addToCart($intProductId, $intAttributeSetId, $intQuantity)
	{
	 ...
	 * @return boolean
	 */
	protected function updateCart($intProductId, $intAttributeSetId, $intQuantity, $blnOverwriteQty = false)
	{
		//Get visitor's cart
	...
	protected function productExistsInCart($intCartId, $intProductId, $intAttributeSetId)
	{
