...
class MediaManager extends Widget
{
	...
	public function generate()
	{
		...
		$objCurrentProductImagePath = $this->Database->prepare("SELECT product_alias, product_sku, product_images, new_import FROM " . $this->strTable . " WHERE id=?")
													 ->limit(1)
		...
		if($objCurrentProductImagePath->new_import > 0)
		{
		...
		elseif($blnDefaultImageSet)
		{
			...
			{	
				...
				$blnShowImageManager = true;
				$fullDestFilePath = $GLOBALS['TL_CONFIG']['isotope_upload_path'] . '/' . $strProductFullPath . '/' . $strProductImage;
