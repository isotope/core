...
/**
 * Class ModuleGiftRegistryReader
 *
 * @copyright  Winans Creative/Fred Bliss 2008 
 * @author     Fred Bliss 
 * @package    Controller
...
class ModuleGiftRegistryReader extends ModuleIsotopeBase
{
	...
	{		
		...
		$arrProductData = $this->getRegistryProductData($arrAggregateSetData, array('product_alias','product_name','product_price', 'product_images'), 'product_name');
	...
	protected function formatProductData($arrProductData)
	{
 		...
 		foreach($arrProductData as $row)
		{
			...
			(
				...
				'add_link'			=> $this->generateActionLinkString('add_to_cart', $row['product_id'], array('aset_id'=>$row['attribute_set_id'],'quantity_requested'=>1), $this->arrJumpToValues['shopping_cart']),
